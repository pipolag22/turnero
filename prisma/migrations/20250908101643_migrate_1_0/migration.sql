...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class WHERE relkind = 'S' AND relname = 'ticket_number_seq'
  ) THEN
...
-- Asegurar que queueNumber use la secuencia como default
ALTER TABLE "Ticket"
  ALTER COLUMN "queueNumber" SET DEFAULT nextval('ticket_number_seq'::regclass);
